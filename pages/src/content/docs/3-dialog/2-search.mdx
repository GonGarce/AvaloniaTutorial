...
En la búsqueda recibimos la lista de álbumes y la convertimos a una lista de ViewModels ``albums.Select(album => new AlbumViewModel(album));``, además de actualizar la propiedad ``IsBusy`` mientras está buscando.
